...
database:set("AshtaR:short"..bot_id..msg.chat_id_,'AshtaR')
end
if text == 'تفعيل الاختصار' then 
send(msg.chat_id_, msg.id_, 1, "تم تفعيل اختصار الروابط في المجموعه 🖲┇", 1, 'html')
database:del("AshtaR:short"..bot_id..msg.chat_id_)
end

local AshtaR = database:get("AshtaR:short"..bot_id..msg.chat_id_)
if not AshtaR then 
local text = msg.content_.text_:gsub('short','اختصار')
if text:match("^(اختصار) (.*)$") then 
local AshtaR2 = {string.match(text, "^(اختصار) (.*)$")}
local req = HTTP.request('http://caut-link.tk/api/?api=a591ca34f771237a385fd9ebadc13e0ec48490f7&url='..URL.escape(AshtaR2[2])..'&alias=')
local data2 = JSON.decode(req)
if not data2.shortenedUrl then  
send(msg.chat_id_, msg.id_, 1, "حدث خطأ 🖲┇", 1, 'html')                 
return false end	
send(msg.chat_id_, msg.id_, 1, "🖲┇ الرابط ~⪼  {"..AshtaR2[2]..'}\n\n 💡┇ الرابط المختصر  ~⪼ {'..data2.shortenedUrl..'}', 1, 'html')                 
end
end
end  -- end text 
end
